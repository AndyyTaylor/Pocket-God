...
Text::Text(int x2, int y2, int w, int h, std::string txt, std::string f) {  // HACK should be implicit initializer
    x = x2;
    y = y2;
    filename = f;
    ...
    loadText(txt, filename);
}

void HUDComponent::loadText(std::string txt, std::string filename) {
    vertices.clear();
    uvs.clear();
